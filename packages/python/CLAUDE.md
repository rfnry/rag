...
- `RoutingConfig.mode`: `QueryMode` enum, default `RETRIEVAL` (backward compat); `DIRECT` is live in R1.2, `HYBRID` is live in R1.3 (requires `hybrid_answerability_model`); `AUTO` raises until R1.4 lands
