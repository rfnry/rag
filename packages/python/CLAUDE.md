...
**RAPTOR retrieval** (R2.3, 2026-04-29) — closes the R2 retrieval-side: `RaptorRetrieval` is a sibling `BaseRetrievalMethod` (alongside `VectorRetrieval` / `DocumentRetrieval` / `GraphRetrieval`) registered into the engine's `MethodNamespace` when `IngestionConfig.raptor.enabled=True`. Default-off keeps existing 1187 R2.2-baseline tests byte-for-byte unchanged: no method registered, no eager registry construction. Lives at `modules/retrieval/methods/raptor.py`. At query time: `knowledge_id is None` short-circuits to `[]` (RAPTOR is per-`knowledge_id` per R2.1's Q2 — cross-scope queries fall through to the other methods); `registry.get_active(knowledge_id)` → `None` also returns `[]` (tree not built yet, RRF fusion proceeds without RAPTOR); otherwise embed query via the configured `BaseEmbeddings` and run a payload-filtered vector search with `knowledge_id == X`, `vector_role == "raptor_summary"`, `raptor_tree_id == active`. The tree-id scope is defence in depth on top of R2.2's blue/green swap: orphan summaries from a prior build cannot leak into results even if GC has not run. `RetrievedChunk` mapping for summaries: `content` = summary text, `chunk_id` = summary point id, `score` = vector-store similarity, `metadata` carries `raptor_tree_id` / `raptor_level` / `raptor_cluster_size` / `raptor_parent_id` / `raptor_reasoning` for trace consumers, `source_id` uses sentinel `f"raptor:{tree_id}"` (NOT `None` — `RetrievedChunk.source_id: str` is non-optional and downstream dedup / source-type weighting may key on it; sentinel keeps those paths well-formed while signalling "summaries span sources by design"), `source_metadata` carries `retrieval_type="raptor"`. Per-method error isolation mirrors the existing methods: registry exceptions, embedding failures, and search failures all log at WARNING and return `[]` rather than raise — degrades to "RRF fusion without RAPTOR" instead of breaking the whole query. Engine wiring (`RagEngine._initialize_impl`): when `ingestion.raptor.enabled=True` AND vector_store + embeddings + `SQLAlchemyMetadataStore` are all configured, the engine eagerly constructs `RaptorTreeRegistry` (so `RaptorRetrieval` can hold a stable reference) and registers a `RaptorRetrieval` instance in the namespace. The builder stays lazy (constructed on first `build_raptor_index` call) — only the registry moved to eager. Soft-skip with WARNING when raptor is enabled but dependencies are missing rather than raising — `build_raptor_index` already raises `ConfigurationError` at the API boundary, and raising at init would break engines that pre-declare `raptor.enabled=True` but defer wiring. The `RaptorRetrieval(weight=...)` constructor parameter flows into RRF fusion via `BaseRetrievalMethod.weight`; the e2e suite includes a regression guard varying the weight between two parallel runs and asserting the fused RAPTOR chunk scores reflect the change (addresses the R5.2-style override-semantics risk). `RetrievalTrace.per_method_results["raptor"]` populates when RAPTOR ran (R8.1's per-method aggregation seeds every declared method, so "ran-and-empty" vs "not configured" both record legibly). Public API: `RaptorRetrieval` re-exported from `rfnry_rag.retrieval.__init__` alongside the other method classes so consumers can build custom pipelines without going through `RagEngine`.
