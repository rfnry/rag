...
- `IterativeRetrievalConfig` (R6.1, scaffold only): `enabled` defaults False (plumbing only — hop loop lands in R6.2, post-loop DIRECT escalation in R6.3); `max_hops` `1 ≤ n ≤ 10` default 3; `gate_mode` allowlist `{"type", "llm"}` default `"type"`; `escalate_to_direct` defaults True (R6.3); `grounding_threshold` `0.0 ≤ t ≤ 1.0` when set, default `None` (inherits `GenerationConfig.grounding_threshold` at consume time); cross-field rule: `enabled=True AND gate_mode="llm"` requires `decomposition_model` (validated in `__post_init__`). Lives at `modules/retrieval/iterative/config.py`; sibling subpackage to `RetrievalService`, not folded in. Raises `ValueError` (not `ConfigurationError`) — matches the spec's tests
