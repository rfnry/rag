...
from rfnry_knowledge.cli.utils import OutputMode as OutputMode
from rfnry_knowledge.cli.utils import get_output_mode as get_output_mode

if TYPE_CHECKING:
    from rfnry_knowledge.generation.models import QueryResult
    from rfnry_knowledge.models import Chunk, HealthSummary, RetrievedChunk, Source, SourceStats
...
def print_health(health: HealthSummary) -> None:
    ...
    if health.retrieval is None:
        ...
        if r.grounding_rate is None:
            ...
            print(f"  Retrieval: {r.total_hits} hits ({r.grounding_rate:.0%} grounded, {r.ungrounded_hits} ungrounded)")
