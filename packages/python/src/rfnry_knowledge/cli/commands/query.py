...
from rfnry_knowledge.cli.output import OutputMode, print_error, print_json, print_query_result
...
async def _query(server_config, text, knowledge_id, min_score, session_name, mode):
    from rfnry_knowledge.cli.commands.session import load_session, save_turn
    ...
    try:
        async with KnowledgeEngine(server_config) as engine:
            result = await engine.query(text, knowledge_id=knowledge_id, history=history, min_score=min_score)
