...
class AnalyzedIngestionService:
    ...
    async def analyze(
        ...
        if ext == ".pdf":
            page_analyses = await self._analyze_pdf_with_cache(file_path, page_filter, knowledge_id, notes=notes)
            file_type = "pdf"
    ...
    async def _analyze_pdf_with_cache(
        ...
        Pages whose rendered-image hash matches an existing ``knowledge_page_analyses`` row
        ...
        if vision_pages:
            ...
            fresh_results = await asyncio.gather(*(self._analyze_one(p, sem, notes=notes) for p in vision_pages))
