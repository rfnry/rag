...
``IterativeRetrievalService`` populates both. The ``termination_reason``
union includes ``"low_confidence_escalated"`` and
...
class IterativeHopTrace:
    ...
    call. The classifier verdict and expansion keys land here so a
...
class IterativeOutcome:
    ...
    - ``"low_confidence_escalated"`` — the loop finished with
    ...
    - ``"low_confidence_no_escalation"`` — the loop finished with
