...
from rfnry_rag.retrieval.cli.constants import CONFIG_DIR, CONFIG_FILE, ENV_FILE
...
def init() -> None:
    ...
    if created:
        ...
        for path in created:
            click.echo(f"  {path}")
        click.echo("\nEdit these files, then run 'rfnry-rag retrieval status' to verify.")
    else:
        click.echo("\nAlready initialized. Edit files or run 'rfnry-rag retrieval status'.")
