...
async def test_compute_entity_id_deterministic():
    from rfnry_rag.retrieval.stores.graph.neo4j import _compute_entity_id
...
async def test_add_entities_empty_list_is_noop():
    ...
    with _patch("rfnry_rag.retrieval.stores.graph.neo4j.AsyncGraphDatabase") as mock_neo4j_module:
        driver = AsyncMock()
        session = AsyncMock()
        ctx = MagicMock()
        ctx.__aenter__ = AsyncMock(return_value=session)
