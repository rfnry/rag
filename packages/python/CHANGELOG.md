...
- **Structured observability + telemetry.** Always-on `Observability` (qualitative event stream) and `Telemetry` (row-per-transaction record) on `RagEngineConfig`. Default `JsonlStderrSink` writes one JSON line per record / row. Concrete sinks ship: `JsonlStderrSink`, `JsonlFileSink`, `MultiSink`, `NullSink`, `RecordingSink`, plus `SqlAlchemyTelemetrySink` for durable consumption via `SQLAlchemyMetadataStore` (two new tables: `rag_query_telemetry`, `rag_ingest_telemetry`). Every LLM call site (text generation, embeddings, vision, reranking, contextualize, document expansion, all five BAML functions) emits `provider.call` / `provider.error` and accumulates raw token counts onto the active row. Retrieval and ingestion methods emit `retrieval.method.*` / `ingestion.method.*` and update per-method timings. No pricing or cost calculation — token counts only.
