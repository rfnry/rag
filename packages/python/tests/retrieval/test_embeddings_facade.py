...
def test_embeddings_unsupported_provider_raises():
    provider = LanguageModelProvider(backend="unknown", model="m", api_key="k")
    with pytest.raises(ConfigurationError, match="Unsupported embeddings provider"):
        Embeddings(provider)


def test_embeddings_model_property_delegates():
    provider = LanguageModelProvider(backend="openai", model="text-embedding-3-large", api_key="sk-test")
    embeddings = Embeddings(provider)
