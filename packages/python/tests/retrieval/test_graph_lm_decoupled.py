...
def test_graph_store_without_ingestion_lm_client_does_not_raise() -> None:
    ...
    config = KnowledgeEngineConfig(
        ingestion=IngestionConfig(methods=[EntityIngestion(store=graph_store)]),
        retrieval=RetrievalConfig(methods=[EntityRetrieval(store=graph_store)]),
    )
...
def test_graph_store_with_lm_client_still_valid() -> None:
    graph_store = MagicMock()
    with patch("rfnry_knowledge.ingestion.methods.entity.build_registry", return_value=MagicMock()):
        config = KnowledgeEngineConfig(
            ingestion=IngestionConfig(
                methods=[EntityIngestion(store=graph_store, provider_client=MagicMock())],
            ),
            retrieval=RetrievalConfig(methods=[EntityRetrieval(store=graph_store)]),
        )
        KnowledgeEngine(config)._validate_config()
